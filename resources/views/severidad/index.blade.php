...
@section('contenido')
    ...
    <div class="container section-title" data-aos="fade-up">
        <h2>Severidad de Anemia</h2>
        <p>Para saber la severidad de tu anemia,
            por favor ingresa tus datos a continuación.</p>
    </div><!-- End Section Title -->
    
    <div class="container" data-aos="fade-up" data-aos-delay="100">

        <form  method="post" role="form" id="predictionForm3">
            @csrf
            <input type="hidden" name="content" id="content">
            <div class="row" style="align-content: center,center">
                <div class="col-md-6 form-group mt-3 mt-md-0">
                    <label for="dni">DNI (8 dígitos)</label>
                    <input type="number" name="dni" class="form-control" id="dni" placeholder="DNI (8 dígitos)" oninput="validateLength(this)" required>
                    @error('dni')
                ...
                <div class="col-md-6 form-group">
                    ...
                    <input type="text" name="nombre" class="form-control" id="nombre" placeholder="Nombre y Apellido" oninput="validateName(this)" required>
                    @error('nombre')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                ...
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    ...
                    <input type="number" class="form-control" step="0.01" name="peso" id="peso" placeholder="Peso (kg)" required>
                    @error('peso')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    ...
                    <input type="number" class="form-control" step="0.01" name="altura" id="altura" placeholder="Altura (cm)" required>
                    @error('altura')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-4 form-group">
                    ...
                    <select name="sexo" class="form-control" id="sexo" required>
                        <option value="" disabled {{ old('sexo') ? '' : 'selected' }}>Seleccionar Sexo</option>
                        <option value="M" {{ old('sexo') == 'M' ? 'selected' : '' }}>Hombre</option>
                        <option value="F" {{ old('sexo') == 'F' ? 'selected' : '' }}>Mujer</option>
                    </select>
                </div>
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    <label for="hmg">Hmg (7-18.5 g/dl)</label>
                ...
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    <label for="pcv">PCV (29-60 %)</label>
                    <input type="number" class="form-control" step="0.01" name="pcv" id="pcv" placeholder="PCV" required>
                    @error('pcv')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    <label for="mcv">MCV (77-119 fL)</label>
                    <input type="number" class="form-control" step="0.01" name="mcv" id="mcv" placeholder="MCV" required>
                    @error('mcv')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4 form-group">
                    <label for="mch">MCH (25-36 g/dL)</label>
                    <input type="number" class="form-control" step="0.01" name="mch" id="mch" placeholder="MCH" required>
                    @error('mch')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    <label for="mchc">MCHC (30-36 mg/dl)</label>
                    <input type="number" class="form-control" step="0.01" name="mchc" id="mchc" placeholder="MCHC" required>
                    @error('mchc')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="row">
                <div class="col-md-4 form-group mt-3 mt-md-0">
                    <label for="rdw">RDW (11.5-14.5 %)</label>
                    <input type="number" class="form-control" step="0.01" name="rdw" id="rdw" placeholder="RDW" required>
                    @error('rdw')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
                <div class="col-md-4 form-group">
                    <label for="tlc">TLC (4.5-38.0 mil/µL)</label>
                    <input type="number" class="form-control" step="0.01" name="tlc" id="tlc" placeholder="TLC" required>
                    @error('tlc')
                        <div class="alerta_perso">{{ $message }}</div>
                    @enderror
                </div>
            ...
            <div class="mt-3 text-center">
                ...
                <script>
                    ...
                    document.getElementById('predictionForm3').addEventListener('submit', function (event) {
                        ...
                        const pcvInput = document.getElementById('pcv');
                        ...
                        const tlcInput = document.getElementById('tlc');
                        
                        const edad = parseInt(edadInput.value);
                        const peso = parseFloat(pesoInput.value);
                        const altura = parseFloat(alturaInput.value);
                        const hmg = parseFloat(hmgInput.value);
                        const pcv = parseFloat(pcvInput.value);
                        ...
                        const tlc = parseFloat(tlcInput.value);
                        ...
                        // Validar PCV
                        if (isNaN(pcv) || pcv < 29 || pcv > 60) {
                            Swal.fire({
                                title: 'Error de Validación',
                                text: "Ingresa los datos del hematocrito dentro del rango de 29 y 60",
                                icon: 'error',
                                confirmButtonText: 'Aceptar'
                            }).then(() => {
                                pcvInput.value = '';
                        ...
                        // Validar MCV
                        if (isNaN(mcv) || mcv < 77 || mcv > 119) {
                            Swal.fire({
                                title: 'Error de Validación',
                                text: "Ingresa los datos del volumen corpuscular medio (MCV) dentro del rango de 77 y 119",
                                icon: 'error',
                                confirmButtonText: 'Aceptar'
                            }).then(() => {
                                mcvInput.value = '';
                        ...
                        // Validar la MCH
                        if (isNaN(mch) || mch < 25 || mch > 36) {
                            Swal.fire({
                                title: 'Error de Validación',
                                text: "Ingresa los datos de la hemoglobina celular media (MCH) dentro del rango de 25 y 36",
                                icon: 'error',
                                confirmButtonText: 'Aceptar'
                            }).then(() => {
                                mchInput.value = '';
                        ...
                        // Validar la MCHC
                        if (isNaN(mchc) || mchc < 30 || mchc > 36) {
                            Swal.fire({
                                title: 'Error de Validación',
                                text: "Ingresa los datos de la concentracion de hemoglobina corpuscular media (MCHC) dentro del rango de 30 y 36",
                                icon: 'error',
                                confirmButtonText: 'Aceptar'
                            }).then(() => {
                                mchcInput.value = '';
                        ...
                        // Validar RDW
                        if (isNaN(rdw) || rdw < 11.5 || rdw > 14.5) {
                            Swal.fire({
                                title: 'Error de Validación',
                                text: "Ingresa los datos de ancho de distribucion de globulos rojos (RDW) dentro del rango de 11.5 y 14.5",
                                icon: 'error',
                                confirmButtonText: 'Aceptar'
                            }).then(() => {
                                rdwInput.value = '';
                        ...
                        // Validar TLC
                        if (isNaN(tlc) || tlc < 4.5 || tlc > 38) {
                            Swal.fire({
                                title: 'Error de Validación',
                                text: "Ingresa los datos del recuento total de leucocitos (TLC) dentro del rango de 4.5 y 38",
                                icon: 'error',
                                confirmButtonText: 'Aceptar'
                            }).then(() => {
                                tlcInput.value = '';
                        ...
                        $.ajax({
                            url: "{{ route('predecir') }}", // Ruta del controlador
                            method: 'POST',
                            headers: {
                            ...
                            success: function(response) {
                                ...
                                setTimeout(() => {  
                                    Swal.close();
                                    Swal.fire({
                                    title: 'Predicción del Grado de Severidad de Anemia',
                ...
                {{-- @if (session('prediccion'))
                    <script>
                        document.addEventListener('DOMContentLoaded', function() {
                            Swal.fire({
                                title: 'Predicción del grado de Anemia',
                                text: "La predicción es: {{ session('prediccion') }}",
                                icon: 'info',
                                confirmButtonText: 'Aceptar'
                            }).then((result) => {
                                if (result.isConfirmed) {
                                    Swal.fire({
                                        ...
                                        if (result.isConfirmed) {
                                            ...
                                            var sexo = $("#sexo").val() == 'M' ? 'Hombre' : 'Mujer';
                                            var hmg = $("#hmg").val();
                                            ...
                                            var tlc = $("#tlc").val();
                                            var plt = $("#plt").val();
                                            var resultado = "{{ session('prediccion') }}";

                                            // Construye la cadena de texto con los datos del formulario
                                            var contentText = `Nombre: ${nombre}, 
                                                DNI: ${dni}, 
                                                Fecha: ${fecha}, 
                                                Hora: ${hora}, 
                                                Edad(meses): ${edad}, 
                                            ...
                                            // Asigna la cadena de texto al campo oculto 'content'
                                            $("#content").val(contentText);
                                            ...
                                            $.ajax({
                                                ...
                                                Swal.fire('Enviado', 'La información ha sido enviada a WhatsApp', 'success');
                                            }).fail(function (err) {
                                                Swal.fire('Error', 'Hubo un problema al enviar la información', 'error');
                                            });
                                        }
        ...
        fetch(`/buscar-registro/${dni}`)
            ...
            .then(data => {
                if (data) {
                    ...
                    document.getElementById('mch').value = data.mch;
                    document.getElementById('mchc').value = data.mchc;
                    document.getElementById('rdw').value = data.rdw;
                    document.getElementById('tlc').value = data.tlc;
                }
            }).catch(error => {
